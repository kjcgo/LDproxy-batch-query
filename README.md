...
# Background
  ...
  Variants often occur dependently of each other. Linkage disequilibirum (LD) is when genetic variants are statistically likely to be present together due to being in close enough physical approximation that they are inherited in a block and are not separated in the process of recombination or "crossing over."
...
# Objective
  ...
  This project aims to conduct a batch query to retrieve associated variants with $R^2 ≥ .01$ through the LDLink API, then parsing the resulting data to remove variants that do not meet the threshold. 
